...
bool menu (HDC choose, HDC menuPic, bool* nachalo_igry, int red, int green, int blue,HDC user, int x, int y, HDC about ) {
    ...
    if (txMouseX() > 300 and   //начать выбранный уровень
        txMouseY() > 385 and
        txMouseY() < 397 and
        txMouseX() < 447 and
        txMouseButtons() & 1)
    ...
    else if (txMouseX() > 283 and   //выход из игры
        txMouseY() > 485 and
        txMouseY() < 503 and
        txMouseX() < 482 and
        txMouseButtons() & 1)
    ...
    else if (txMouseX() > 262 and  //выбор уровней
        txMouseY() > 306 and
        txMouseY() < 415 and
        txMouseX() < 708 and
        txMouseButtons() & 1)
    ...
    else if (txMouseX() > 289 and    //about - дополнительно
        txMouseY() > 526 and
        txMouseY() < 536 and
        txMouseX() < 486 and
        txMouseButtons() & 1)
    ...
    else if (txMouseX() > 300 and       //Выбор уровней
        txMouseY() > 437 and
        txMouseY() < 448 and
        txMouseX() < 438 and
        txMouseButtons() & 1)
...
void chooseVoid (HDC menuPic ,HDC choose, int red, int green, int blue, bool game_over, int* level, bool* nachalo_igry, HDC user, int x, int y, HDC about) { //функция выбора уровней
    ...
    while (*nachalo_igry == false) {
       ...
       if (txMouseX() >533 and    //загрузить уровень №2
            txMouseY() > 90 and
            txMouseY() < 329 and
            txMouseX() < 917 and
            txMouseButtons() & 1)
        ...
        if (txMouseX() > 970 and           //загрузить уровень №3
            txMouseY() > 90 and
            txMouseY() < 332 and
            txMouseX() < 1359 and
            txMouseButtons() & 1)
        ...
        if (txMouseX() > 530 and    //загрузить пользовательский уровень
            txMouseY() > 521 and
            txMouseY() < 761 and
            txMouseX() < 917 and
            txMouseButtons() & 1)
        ...
        if (txMouseX() > 29 and      //возврат в меню
            txMouseY() > 830 and
            txMouseY() < 874 and
            txMouseX() < 90 and
            txMouseButtons() & 1)
...
void aboutVoid (HDC menuPic ,HDC choose, int red, int green, int blue, bool game_over, int* level,
 bool* nachalo_igry, HDC user, int x, int y, HDC about) {  //документация
